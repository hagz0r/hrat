...
async def build_client(
    ...
    if process.returncode == 0:
        built_file_path = os.path.join(CLIENT_PROJECT_PATH, "target", "release", "client.exe")
        download_url = f"/builder/download/{client_name}"
        downloadable_file_path = os.path.join(BASE_DIR, "..", client_name)
        if os.path.exists(built_file_path):
            os.rename(built_file_path, downloadable_file_path)

        response_html = f"""
            <div class="bg-green-800 border border-green-500 p-4 rounded-lg">
                <h3 class="font-bold text-lg">Build completed successfuly!</h3>
                <a href="{download_url}" class="text-blue-300 hover:underline" download>Скачать {client_name}</a>
            </div>
